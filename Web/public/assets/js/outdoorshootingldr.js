    ...
    get(que).then((snapshot) => {
        if (snapshot.exists()) {
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                // tempLeaderboardArray.push({key: childSnapshot.key, username: childSnapshot.child("displayname").val(), highestPoints: childSnapshot.child("highestPoints").val()});
            });
        }
    });
}
    ...
    setTimeout(() => {
        get(ref(db, "playerStats/" + key)).then((snapshot) => {
            if (snapshot.exists()) {
                ...
                $("#outdoorGamesDetail").text(`${snapshot.child("outdoorTotalGame").val()}`);
                $("#totalOutdoorDetail").text(`${snapshot.child("outdoorTotalPoints").val()}`);
            }
            else {
